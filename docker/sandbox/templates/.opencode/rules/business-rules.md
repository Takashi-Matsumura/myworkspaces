...
| 外部の最新情報 / 一次情報 | `web_search` (Phase B 以降で導入。未実装の段階では「未確認」と書く) | 推測で書く |
...
| **Web** | `web_search` (Phase B 以降) | `research/<slug>.md` |
...
## DeepSearch 規律 (Web フェーズ; Phase B 以降)

`web_search` ツールが利用できる時期に入ったら、以下を守ってください:

- 1 ターンに `web_search` を呼ぶのは **最大 5 回**
- そのうち本文取得 (`read_url`) を行うのは **最大 2 件**
- 引用は **3 件以上必須**
