...
## 7. Web 調査が必要な時 (Phase F-C)

repo 内の出典 (`path:line`) が分析の本筋だが、フレームワークや言語仕様の **一般的な情報** が必要な時は `web_search` ツールを使ってよい。

- survey フェーズ: 主要依存ライブラリの公式サイトを確認 (技術スタックの呼称統一など)
- detail フェーズ: 外部仕様 (HTTP API の RFC, ORM の挙動仕様) の根拠が必要な時
- port フェーズ: 移植先言語の同等ライブラリ調査 (例: C# の \`HttpClient\` → Go の \`net/http\` 移植)

規律:
- 1 ターンに `web_search` を呼ぶのは **最大 3 回**
- レポート本文の出典は **repo 内の `path:line` を優先**。Web 引用は補助情報として末尾に脚注 `[^N]: <URL>`
- repo を読まずに Web だけで結論を書くのは禁止
