...
export default function CodingHelp({ fontSize = 13 }: { fontSize?: number }) {
  return (
    <div
      className="h-full overflow-y-auto bg-[#0b0b0f]"
      style={{ fontSize: `${fontSize}px`, lineHeight: 1.6 }}
    >
      <div className="mx-auto max-w-3xl space-y-6 px-6 py-5 text-white/85">
...
function Section({
  ...
  return (
    <section>
      <h2
        className="border-b border-emerald-500/30 pb-1 font-semibold text-emerald-300"
...
function Subsection({
  ...
  return (
    <div className="mt-3">
      <h3 className="font-semibold text-white/90" style={{ fontSize: "1em" }}>
        {title}
      </h3>
...
function Faq({ q, children }: { q: string; children: React.ReactNode }) {
  return (
    <details className="rounded border border-white/10 bg-white/5">
      <summary className="cursor-pointer select-none px-3 py-1.5 font-medium text-white/90 hover:bg-white/10">
        Q. {q}
      </summary>
      <div className="border-t border-white/10 px-3 py-2 text-white/70">
        {children}
      </div>
    </details>
  );
...
function Kbd({ children }: { children: React.ReactNode }) {
  return (
    <kbd
      className="rounded border border-white/20 bg-white/10 px-1.5 py-0.5 font-mono text-white/80 shadow-sm"
