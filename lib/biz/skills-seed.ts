...
2. **各サブクエリで \`web_search\` (max_results=5)**
   - 一次情報 (公式 / プレス / 公的統計 / arXiv / 学術機関) を優先
   - 二次情報 (まとめサイト / コピー記事) は採用しない
3. **上位 2 件を \`web_search read_url\` で本文取得**
   - 1 ステップ内で最大 2 件、合計 (5 ステップ) で 5〜10 件
4. **取得ごとに \`research/<slug>.md\` に追記**
...
3. \`<topic>\` のスラグはユーザー指示か、最頻ファイル名から推定
4. 下のフォーマットに沿って \`reports/<topic>-summary.md\` を生成
